...
import net.blay09.balybot.irc.event.IRCExceptionEvent;
...
public class IRCSender implements Runnable {
	...
	public IRCSender(IRCConnection connection) {
	...
	public void run() {
		try {
			while (running) {
				...
				try {
					Thread.sleep(antiFlood ? connection.getConfig().antiFloodTime : 100);
